...
type QueryParams struct {
	...
	ExactTime string `form:"exactTime" json:"exactTime"` // exact time in ISO format (YYYY-MM-DDTHH:mm or YYYY-MM-DDTHH:mm:ss)
...
func getEvents(c *gin.Context) {
	...
	log.Printf("[QUERY] Database: %s, TimeRange: %s, ExactTime: %s, ContentFilter: %s, TopicFilter: %s, Limit: %d, Offset: %d",
		params.Database, params.TimeRange, params.ExactTime, params.Content, params.Topic, params.Limit, params.Offset)
...
func queryClickHouse(params QueryParams) ([]Event, error) {
	log.Printf("[ClickHouse] Starting query with timeRange=%s, exactTime=%s, content=%s, topic=%s", params.TimeRange, params.ExactTime, params.Content, params.Topic)
	...
	// Add time condition - either exact time or time range
	if params.ExactTime != "" {
		// Parse exact time - support both with and without seconds
		var exactTime time.Time
		var err error

		// Try parsing with seconds first (datetime-local can include seconds)
		if len(params.ExactTime) > 16 {
			exactTime, err = time.Parse("2006-01-02T15:04:05", params.ExactTime)
		} else {
...
func queryPostgreSQL(params QueryParams) ([]Event, error) {
	log.Printf("[PostgreSQL] Starting query with timeRange=%s, exactTime=%s, content=%s, topic=%s", params.TimeRange, params.ExactTime, params.Content, params.Topic)
